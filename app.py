...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category', 'App', 'Rating']).Reviews.sum().sort_values(ascending=False).reset_index().head(10).loc[:,['Category', 'App', 'Reviews', 'Rating']].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    free_paid = pd.pivot_table(data=playstore, index='Category', columns='Type', values='Installs', aggfunc='sum', margins=True, margins_name='Total').sort_values(by='Total', ascending=False).reset_index().iloc[1:6, 0:3]
